...
if (args.length < 3) {
    ...
    var removeExt = (x : string) => x.substr(0, x.length - path.extname(x).length);

    var replaceSlash = (x : string) => x.split("\\").join("/");
    ...
    var names = files
        .map(x=> replaceSlash(removeExt(path.relative(base, x))));
